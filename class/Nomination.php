...
class Nomination
{
    ...
    public function __construct($bannerId, $firstName, $middleName, $lastName,
            $email, $asubox, $position, $department, $yearsAtASU, $phone, $gpa,
            $class, $responsibility, $nominatorFirstName, $nominatorMiddleName,
            $nominatorLastName, $nominatorAddress, $nominatorPhone,
            $nominatorEmail, $nominatorRelation, $nominatorUniqueId, $category,
            $period, $alternate_award = null)
    {
        ...
        $this->alternate_award = $alternate_award;
    }
    ...
    public function getLink($text = null)
    {
        ...
        if (is_null($text)) {
            $link = $view->getLink($name . ' - ' . strftime("%B %d, %Y",
                            $nominator->getSubmissionDate()));
        } else {
...
function reportRowForCSV($obj)
{
    ...
    $row['alternate_award'] = $data->alternate_award;
