...
class NominationForm extends \nomination\View
{
    ...
    public function display(Context $context)
    {
        ...
        $tpl['AWARD_DESCRIPTION'] = \PHPWS_Settings::get('nomination',
                        'award_description');
        $currPeriod = Period::getCurrentPeriod();
        $tpl['PERIOD_END'] = $currPeriod->getReadableEndDate();
        $tpl['NUM_REFS'] = \PHPWS_Settings::get('nomination',
                        'num_references_req');
        ...
        if ($vis->isVisible('nominee_responsibility')) {
            $form->addRadioAssoc('nominee_responsibility',
                    array(1 => 'Yes', 2 => 'No'));

            if (isset($c['nominee_responsibility']))
                $form->setMatch('nominee_responsibility',
                        $c['nominee_responsibility']);
        }
        ...
        if ($vis->isVisible('nominee_class')) {
            $form->addDropBox('nominee_class',
                    array(-1 => 'Select', 'fr' => 'Freshmen', 'so' => 'Sophomore', 'jr' => 'Junior', 'sr' => 'Senior', 'grad' => 'Graduate', 'emp' => 'Faculty/Staff'));
            $form->setMatch('nominee_class',
                    isset($c['nominee_class']) ? $c['nominee_class'] : -1);
        ...
        if ($vis->isVisible('category')) {
            ...
            $form->setMatch('category',
                    isset($c['category']) ? $c['category'] : -1);
        }
        ...
        for ($i = 0; $i < $numRefsReq; $i++) {
            ...
            if ($vis->isVisible('reference_first_name')) {
                if (isset($c['reference_id'][$i])) {
                    $form->addHidden('reference_id_' . $i,
                            $c['reference_id'][$i]);
                }
            ...
            if ($vis->isVisible('reference_relationship')) {
                ...
                $form->addCssClass('reference_relationship_' . $i,
                        'form-control');
            }
        ...
        if ($vis->isVisible('statement')) {
            if (!isset($nomination)) {
                ...
                $tpl['STATEMENT'] = NominationDocument::getFileWidget(null,
                                'statement', $form);
            } else {
                $tpl['FILES_DIR'] = PHPWS_SOURCE_HTTP;
                ...
                $tpl['STATEMENT'] = NominationDocument::getFileWidget(null,
                                'statement', $form);
                $tpl['DOWNLOAD'] = $doc->getDownloadLink($c['unique_id'],
                        'Download Statement');
            }
        }
        ...
        if ($vis->isVisible('nominator_middle_name')) {
            ...
            $form->setLabel('nominator_middle_name',
                    'Nominator\'s middle name: ');
        ...
        if ($vis->isVisible('nominator_relationship')) {
            $form->addText('nominator_relationship',
                    isset($c['nominator_relationship']) ? $c['nominator_relationship'] : '');
        ...
        if ($vis->isVisible('alternate_award')) {
            $tpl['alternate_award'] = ' ';
            $alternateArray = explode(',', $c['alternate_award']);
            if (!empty($alternateArray)) {
                foreach ($alternateArray as $val) {
                    $tpl[$val] = 'checked="checked"';
                }
            }
        }
        ...
        $result = \PHPWS_Template::process($tpl, 'nomination',
                        'nomination_form.tpl');
